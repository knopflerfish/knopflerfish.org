	...
	public ComponentDeclaration readXML(ZipEntry zipEntry,JarFile jarFile) throws IllegalXMLException {
				...
				if (zipEntry != null) {
					...
					try{
						...
						throw new IllegalXMLException("can't create inputstream:" +
					...
					/* create the parser */
						try{
							...
							while (parser.nextTag() != XmlPullParser.END_TAG) {
								try{
									...
									if (parser.getName().equals("implementation")) {
										...
										throw new IllegalXMLException("Unsupported tag");
									}
								}catch(IllegalXMLException e){
									errorOccured=true;
									throw new IllegalXMLException("Error parsing in while statement:" +
											e.getMessage(),e.getCause());
								}catch(Exception e){
									throw new IllegalXMLException("Unkown error in CustomParser:" +
											e.getMessage(),e.getCause());
								}
							}
							...
							throw new IllegalXMLException("error in PullParser:" +
									e.getMessage(),e.getCause());
							
						}catch(IOException e){
							throw new IllegalXMLException("FileSystem error:" +
									e.getMessage(),e.getCause());
						}
			...
			for (int i = 0; i < parser.getAttributeCount(); i++) {
				try {
					if (parser.getAttributeName(i).equals("name")) {
						...
						if (parser.getAttributeValue(i) == null) {
							...
							for (int j = 0; j < supportedTypes.length; j++) {
								/*
								 * If the found attribute value equals one of the
								 * supported types
								 */
								if (parser.getAttributeValue(i).toLowerCase()
										.equals(supportedTypes[j].toLowerCase())) {
					...
					System.out.println("Error Parsing reference tag:" + e
							+ " with attribute name:" + parser.getAttributeName(i) ); //TODO Replace with Exceptions
				}
			}
...
//											+ parser.getAttributeValue(i));
			...
			throw new IllegalXMLException("Error Parsing Service:" + e); //TODO Replace with Exceptions
		}
...
//								+ parser.getAttributeValue(i));
			...
			throw new IllegalXMLException("Error Parsing reference tag:" + e); 
		}
...
//									+ parser.getAttributeValue(i));
				...
				throw new IllegalXMLException("Error Parsing implementation tag:" + e); //TODO Replace with Exceptions
			}
		} else {
...
//								+ parser.getAttributeValue(i));
			...
			throw new IllegalXMLException("Error Parsing reference tag:" + e); //TODO Replace with Exceptions
		}
