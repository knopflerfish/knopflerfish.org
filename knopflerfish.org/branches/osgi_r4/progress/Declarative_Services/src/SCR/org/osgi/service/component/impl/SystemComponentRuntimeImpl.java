 ...
 */
public class SystemComponentRuntimeImpl implements BundleListener,ServiceListener {
	...
	private LinkedList activeComponents = new LinkedList();

	/** variable holding inactive components */
	private LinkedList inactiveComponents = new LinkedList();
	...
	/**
	 * this method listens for service changes it will check the type of the event
	 * and perform correct actions depending on the event.
	 */
		...
		if(event.getType()==ServiceEvent.REGISTERED || event.getType()==ServiceEvent.UNREGISTERING){
			try{
				...
				for(int i=0;i<objectClasses.length;i++){
					...
					/* check if the event is a REGISTERED event check if any inactive component has 
					 * this registered event as reference if so try to evalutate the 
					 * that component with the evaluateComponentDeclaration(..)
					 * method. if the component is satisfied,i.e has all dependencies
					 * it needs when it will be activated and its services, if any ,
					 * will be registered into the framework. 
					 */
					if(event.getType()==ServiceEvent.REGISTERED){
						/* go through all inactive components to se if any of them
						 * has a reference to this service 
						 */
						for(int j=0;j<inactiveComponents.size();j++){
							/* get the componentDeclaration */
							ComponentDeclaration componentDeclaration=
								(ComponentDeclaration)inactiveComponents.get(j);
					...
					/* always check all active components  no matter if it is 
					 * a REGISTERED or UNREGISTERED  event if an active component
					 * has a refence to the event perform correct action
					 * i.e, disable the component if its necessary, rebind
					 * the component and call the components activate method.
					 * 
					 */
					for(int j=0;j<activeComponents.size();j++){
						...
						if(activeComponents.get(j) instanceof DeclarativeComponent){
							...
							while (referenceIterator.hasNext()) {
								...
								/*
								 * check if the name of the interface is the same as the newly
								 * registered. Also check that the cardinality is 0..n or
								 * 1..n. Also ensure that a bind method is declared
								 * If that is the case the active component should be
								 * binded or rebinded again. This should only be done 
								 * if the event is a REGISTERED event
								 */
								if(referenceInfo.getInterfaceType().equals(objectClass) &&
								...
								if((event.getType()==ServiceEvent.UNREGISTERING) &&
									...
									/* check if it is a static policy. Static policy will not try
									 * to dynamically rebind the component SCR will instead 
									 * disable the component and try to create a new one. 
									 */
									if(policy.equals("static")){
										 ...
										 * TODO check if the evaluation method should care about
									...
									/*
									 * dynamic policy is a little bit more complex it will behave diffrent 
									 * depending on the cardinality declared. if the cardinality is 1..1 and no
									 * references are available then the component declaring dependency to this
									 * unregistered service cannot work with out a rebind action. If the cardinality is 
									...
									if(policy.equals("dynamic") && component.getComponentDeclaration().
										...
										if(cardinality.equals("1..1") || cardinality.equals("0..1")){
											 ...
											 * do only have one instance of the component object
											 */
											if(component instanceof ImmediateComponent || 
												...
												/* ensure that the context isn't null 
												 * if so it means that the service never been requested and 
												 * the component is of the type CustomDelayedService. If 
												 * the context is null then there are no instance(s) to
												 * rebind.
												 */
												
												if(componentContext!=null){
													/* create a string representing the unbind method */
													String unbindMethod = referenceInfo.getUnbind();
													/* create a string representing the bind method */
													...
													try{
														...
														/* print the error */
														System.err.println("error calling activate method due to:\n"+ 
																e);
													}
													
												}// end if(componentContext!=null){
												
											}else if(component instanceof CustomComponentServiceFactory && serviceObject!=null){
											...
											if(cardinality.equals("1..1") && serviceObject==null){
												/*
												 * in this case disable the component it should not work 
										 ...
										 * references of the given type and MUST have at least
										 * one reference bounded. 0..n means that the component
										 * can work with out a reference and should not be disabled if
										...
										if(cardinality.equals("1..n") || cardinality.equals("0..n")){
											...
											if(component instanceof ImmediateComponent || 
												 ...
												 * here we need all the component instances 
												 */
												Vector contexts = component.getComponentContexts();
												try{
													/* get the references */
							...
							System.err.println("error in SystemComponentRuntimeImpl.serviceChanged(..) due to:\n"+ 
	 ...
	 * ComponentDeclaration this is usually the main entrance for a
	 * bundle having declarative components. Further the method will
	 * parse the declared xml files and create ComponentDeclarations of
	 * the files. If the event is a stop event then the method will
	 * check if the bundle has declarative components and if they are
	 * activated. If that is the case the configurations will be disabled
	 * and unregistered from the framework. 
	 */
	public synchronized void bundleChanged(BundleEvent event) {
		...
		if (event.getBundle().getHeaders().get("Service-Component") != null
			...
			try{
				...
				System.out.println("The Declared components are:" +(String) event.getBundle().getHeaders().get(
						"Service-Component") );
				...
				for(int i=0;i<manifestEntries.length;i++){
					...
					if(zipEntry!=null){
						...
						try{
							...
							/* evaluate the component don't overide the isAutoEnable() attribute */
							evaluateComponentDeclaration(componentDeclaration,false);
							
						}catch(ComponentException e){
							/* print the error */
							System.err.println("error when evaluating started bundle with component\n" + 
									"The following Exception(s) was thrown:\n" +e);
						}
					}else{
						System.err.println("error can't find:" + manifestEntries[i] );
		...
		}// end if (event.getBundle().getHeaders().get("Service-Component") != null
		//&& event.getType() == BundleEvent.STOPPED) 
	...
	/**
	 * this method will dispose a given component configuration it will search through all
	 * active components and if a match on handed object is found the method will
	 * unbind, deactivate and unregister the services if there are any.
	 *  
	 * @param componentObject the component object to be disposed
	 * @throws ComponentException if failed during the process 
	 */
	public  void disposeComponent(Object componentObject) throws ComponentException{
		...
		for(int i=0;i<activeComponents.size();i++){
			 ...
			 * component. Those are quite simular to each other they
			 * both have ONE component instance and ONE componentContext. The
			 * only diffrence is that an CustomDelayedService offers Services 
			 * to other. 
			 */
			if(component instanceof ImmediateComponent || component instanceof CustomDelayedService){
				/* get the component element located in the componentContext */
				Object element = component.getComponentContext().getComponentInstance().getInstance();
				
				/* check if the component is equal to the passed componentObject variable */
					...
					try{
						...
						try{
							...
							try{
								...
								throw new ComponentException("error unregistering " +
										component.getComponentDeclaration().getComponentName() +
										" due to:\n" + e, e.getCause());
							}
						}catch(ComponentException e){
							throw new ComponentException("error when deactivating  " +
									component.getComponentDeclaration().getComponentName() +
									" due to:\n" +e, e.getCause()); 
						}
			...
			/* this case is moore tricky. The service factory 
			 * do have many instances and contexts. Those are created 
			 * when a request for the service is done. Therefore all contexts and
			 * instances have to be eliminated here.
			 */
			if(component instanceof CustomComponentServiceFactory){
				...
				for(int j=0;j<contexts.size();j++){
					/* get the component element located in the componentContext */
					Object element = ((ComponentContext)contexts.get(j)).getComponentInstance().getInstance();
	...
	/**
	 * this method will disable a component or components after a given name
	 * it will ensure that the component is in the same bundle as the bundle calling
	 * the method. The method cannot just only check if its a DeclarativeComponent type.
	 * It has to check  the implementation of the DeclarativeComponent because it 
	 * will perfom diffrent actions depending on the implementation that is to be disabled.
	 * 
	 * 
	 * @param componentName the name of the component(s) to be disable
	 * @param requestBundle the bundle the requesting component is located in
	 */
	public void disableComponent(String componentName, Bundle requestBundle,boolean isStopping) throws ComponentException{
		...
		for (int i = 0; i < activeComponents.size(); i++) {
			...
			if (object instanceof ImmediateComponent) {
				...
				if (componentDeclaration.getComponentName().equals(
					...
					/* check that the requesting bundle is in the same bundle as this
					 * component declared int the componentDeclaration
					 */
					if (componentDeclaration.getDeclaringBundle().equals(
						...
						try{
							...
							try{
								/* inactivate the component use the components own context */
								inactivateInstance(component.getComponentContext());
								...
								/* throw the error */
								throw new ComponentException("error when deactivating "+componentDeclaration.
										getComponentName()+" due to:\n"+ e,e.getCause());
							}
							
						}catch(ComponentException e){
							/* throw the error */
							throw new ComponentException("error when unbinding "+componentDeclaration.
									getComponentName()+" due to:\n"+ e, e.getCause());
						}
						
					}// end if(componentDeclaration.getDeclaringBundle().equals(requestBundle))

				}// end if(componentDeclaration.getComponentName().equals(componentName))
				...
				if (componentDeclaration.getComponentName().equals(
					...
					if (componentDeclaration.getDeclaringBundle().equals(
						...
						try{
							 ...
							 * that might happen if this component is unrequested
							 */
							if(componentContext!=null){
							...
							try{
								...
								if(componentDeclaration.getDeclaringBundle().getRegisteredServices().length>0){
									...
									/* an Illegal state may occure if a bundle STOP event causes this
									 * and the component is to be disable is in the same bundle
									 * as the STOPPED bundle. 
									 */
									try{
										component.getServiceRegistration().unregister();
									}catch(IllegalStateException e){
										/* do nothing the service will disappear when 
								...
								/*this happens if something goes wrong unregistering the service
								 * print the error 
								 */
								e.printStackTrace();
								throw new ComponentException("error unregistering the service for " +
										componentDeclaration.getComponentName() +" due to:\n" + e,
										e.getCause());
								
							}
						}else{
							/* this component has never been requested and there fore
							 * no context and instance is available
							...
							if(componentDeclaration.getDeclaringBundle().getRegisteredServices().length>0){
								...
								try{
									...
									/* do nothing this is caused because the
									 * method is called during a STOP event 
									 */
									//System.err.println(e);
								}
							}
			...
			if (object instanceof CustomComponentServiceFactory) {
				...
				if (componentDeclaration.getComponentName().equals(
					...
					if (componentDeclaration.getDeclaringBundle().equals(
							...
							/* get the vector with all component contexts created by this
							 * configuration
							 */
							Vector contexts =  component.getComponentContexts();
							...
							for(int j= 0;j<contexts.size();j++){
								...
								try{
									...
									try{
										...
										/* remove the object from active components vector */
										activeComponents.remove(object);
										/* check that the component declaration not already exists here */
										if(!inactiveComponents.contains(component.getComponentDeclaration())){
											/* add the declaration to inactive components vector */
											inactiveComponents.add(component.getComponentDeclaration());
										}
					    ...
					    try{
							...
							/* remove the object from active components vector do this here as well
							 * because sometimes the for statement will be skipped, i.e, the service
							 * has never been requested it is IMPORTANT TO REMOVE THIS before 
							 * unregistration. During unregistration will the SCR
							 * check the array again and it should not find this object
							 */
							activeComponents.remove(object);
							 ...
							 * print the error 
							 */
							throw new ComponentException("error unregistering the component service factory " +
									componentDeclaration.getComponentName() +":" + e,e.getCause());
				...
				if (componentDeclaration.getComponentName().equals(
					...
					if (componentDeclaration.getDeclaringBundle().equals(
						...
						try{
							...
							component.getServiceRegistration().unregister();
							/* make sure it doesn't already exits in the vector */
							if(!inactiveComponents.contains(component.getComponentDeclaration())){
							...
							throw new ComponentException("error unregistering the component factory " +
									componentDeclaration.getComponentName() +" due to:\n" + e,
									e.getCause());
						}
	 ...
	 * ensure that the component is in the same bundle as the requesting component is.
	 * 
	 * @param componentName the name of the component(s) to be enabled null enables all in the bundle
	 * @param requestBundle the bundle the requesting component is located in 
	 */
	public void enableComponent(String componentName,Bundle requestBundle)
		...
		for(int i=0;i<inactiveComponents.size();i++){
			try{
				/* create a componentDeclaration variable */
				ComponentDeclaration componentDeclaration = 
					(ComponentDeclaration)inactiveComponents.get(i);
				...
				if(componentDeclaration.getComponentName().equals(componentName) || 
					...
					if(componentDeclaration.getDeclaringBundle().equals(requestBundle)){
						try{
							/* evalute the declaration override the componentdeclarations 
							 * isAutoEnabled() 
							 */
							evaluateComponentDeclaration(componentDeclaration,true);
				...
				throw new ComponentException("error in enableComponent(..) due to:\n" +e ,e.getCause());
			}// end try
	...
	/** 
	 * locates a service with a given name declared in the reference element of a
	 * component declaration. This method will use the internal bundle context to
	 * locate services.
	 * 
	 ...
	 */
	public Object locateService(String serviceName,String componentName) throws ComponentException{
		
		/* TODO Check if ID is needed here to identify the component 
		 * split up in small methods
		 */
			...
			if(activeComponents.get(i) instanceof DeclarativeComponent){
				...
				if(component.getComponentDeclaration().getComponentName()
					...
					while(referenceIterator.hasNext()){
						...
						/* check if the the declared reference name is equal to the given 
						 * 'serviceName' 
						 */
						if(referenceInfo.getReferenceName().equals(serviceName)){
							...
							ServiceReference serviceReference=
								bundleContext.getServiceReference(interfaceName);
				...
				}// end if(component.getComponentDeclaration().getComponentName()
				//.equals(componentName))
	 ...
	 * @return object the service object
	 */
	public Object[] locateServices(String serviceName,String componentName) throws ComponentException{
					...
					while(referenceIterator.hasNext()){
						...
						if(referenceInfo.getReferenceName().equals(serviceName)){
							try{
								...
								/* locate the services */
								ServiceReference[] serviceReferences=
									bundleContext.getServiceReferences(interfaceName,null);
								
								/* create a an array with the same size as the service
								 * references
								 */
								...
								if(serviceReferences!=null){
									...
									for(int j=0;j<serviceReferences.length;j++){
										...
										if(service!=null){
											/* assign the service to the current position in the array */
											services[j] = service; 
										}else{
											/* throw this if service is null should not happen */
											throw new ComponentException("fatal internal error in locateServices(..)"+
	 ...
	 * @param overideEnable if the method should over care about the component
	 * 			declarations isAutoEnable() attribute.
	 */
	private synchronized void  evaluateComponentDeclaration(
			ComponentDeclaration componentDeclaration,boolean overideEnable) throws ComponentException {
			...
			if (componentDeclaration.getFactory() != null) {
				...
				if (isSatisfied(componentDeclaration)
					...
					if(!inactiveComponents.contains(componentDeclaration)){
						System.out.println("*************** " + componentDeclaration.getComponentName() +
								" is not satisfied **********");
						/* Save data here */
				...
				if (isSatisfied(componentDeclaration)) {
					...
					if (componentDeclaration.getServiceInfo().size() > 0) {
						...
						String[] allInterfaces = new String[vectorInterfaces.size()];
						...
						if (componentDeclaration.isServiceFactory()) {
							...
							try {
									...
									/* add the service factory to the vector of active components */
							...
							try {
								/*
								 * register custom delayed service
								 * here
								 */
								DeclarativeComponent delayedComponent = registerDelayedComponent(allInterfaces,
											componentDeclaration);
								
								/* add the delayed service to the vector of active components */
					...
					System.out.println("********* "+ componentDeclaration.getComponentName()+ " is not satisfied ************");
	 ...
	 * doesn't declare an activate method. Normally a NoSuchMethodException
	 * is raises, but this has to be ignored.
	 * 
	...
	private void inactivateInstance(ComponentContext componentContext)
		...
		try {
			...
			System.out.println("************* trying to invoke "
					+ componentInstance.toString() + ".deactivate(...) method ");
	...
	private CustomComponentFactory registerComponentFactory(
		...
		try {
			...
			ServiceRegistration registration = componentDeclaration.getDeclaringBundle()
	...
	private CustomDelayedService registerDelayedComponent(String[] interfaceNames,
		...
		/* create a service factory */
		CustomDelayedService delayedService =  new CustomDelayedService(interfaceNames,
					componentDeclaration);
	...
	/**
	 * this method will invoke an active component configurations bind or unbind method and
	 * pass the new object to it.
	 * 
	 * @param serviceObject the service object to be passed as argument
	 * @param instance the target instance which should retrive the serviceObject 
	 * @param methodName the name of the bind method
	 * @param interfaceName the interfaceName
	 */
	private void reInvokeReference(Object serviceObject,Object instance,String methodName,String interfaceName){
	...
	private class CustomDelayedService implements ServiceFactory,DeclarativeComponent {
	...
	private class ImmediateComponent implements DeclarativeComponent {
		...
		public ImmediateComponent(ComponentDeclaration declaration) {
			...
			try {
				...
				/* create the context in this case a request will come frome the bundle declaring the
				 * component
				 */
		 ...
		 * not used here
		 */
		public ServiceRegistration getServiceRegistration() {return null;}
	 ...
	 * @author Magnus Klack
	 */
	private class CustomComponentServiceFactory implements ServiceFactory,DeclarativeComponent {
			...
			try {
				...
				if (componentConfiguration == null) {
					...
					try {
						...
						/* create a new componentContext request will come  
						 * from the same bundle declaring the component
						 */
		...
		/**
		 * this method returns all created componentContexts created by this service factory
		 * 
	 ...
	 * @author Magnus Klack
	 */
	private class CustomComponentFactory implements ComponentFactory,DeclarativeComponent {
		...
		/**
		 * this method will create a new Component of the type which this instance's 
		 * component declaration declares. It can be an delayed component or an
		 * immediate component
		 * 
		...
		public ComponentInstance newInstance(Dictionary properties) throws ComponentException{
			...
			if (isSatisfied(componentDeclaration)) {
				 ...
				 * declaring the bundle use the declaring bundle in the component
				 * declaration. 
				 */
				ComponentContext componentContext = createComponentContext(
						componentDeclaration, componentCounter,
