...
rsync -r -a -v -e ssh -i $PRIVATE_KEY "${KF_USER}@${KF_SERVER}:${MVN_REMOTE_DIR}" $MVN_LOCAL_REMOTE_DIR

run_gradle

rsync -r -a -v -i -e ssh -i $PRIVATE_KEY $MVN_LOCAL_REMOTE_DIR/ "${KF_USER}@${KF_SERVER}:${MVN_REMOTE_DIR}"
